...
## Container setup (optional, reproducible for teams)

Requires Docker daemon access (rootless Docker or user in `docker` group). If you don't have that, use the pip/conda workflow above.
...
2. Run:
   ...
   Or via compose:
   ```bash
   docker compose up -d isaacsim-go2
   docker compose exec isaacsim-go2 bash
   ```
...
Persistence:
- The repo is mounted into `/workspace/repo-go2`, so edits on host are immediately visible in the container.
- Conda envs/pkgs are stored in named volumes (`isaacsim_go2_conda_envs`, `isaacsim_go2_conda_pkgs`) so anything you install in `conda activate isaaclab` survives restarts.
- We no longer run the container with `--rm`; re‑enter with `docker start -ai isaacsim_go2`.
- To reset everything: `docker rm -f isaacsim_go2` and (optionally) `docker volume rm isaacsim_go2_conda_envs isaacsim_go2_conda_pkgs`.
